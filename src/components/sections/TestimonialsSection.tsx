...
// Memoized testimonial card to prevent re-renders
...
export default function TestimonialsSection() {
  ...
  const handleSelect = useCallback(() => {
    if (!api || typeof api !== 'object' || !('selectedScrollSnap' in api) || typeof api.selectedScrollSnap !== 'function') return;
    setCurrent(api.selectedScrollSnap());
  }, [api]);

  useEffect(() => {
    if (!api || typeof api !== 'object' || !('on' in api) || typeof api.on !== 'function') return;

    api.on("select", handleSelect);
    api.on("settle", handleSelect);

    return () => {
      if (!api || typeof api !== 'object' || !('off' in api) || typeof api.off !== 'function') return;
  ...
  const handleScrollTo = useCallback((index: number) => () => {
    if (!api || typeof api !== 'object' || !('scrollTo' in api) || typeof api.scrollTo !== 'function') return;
    api.scrollTo(index);
  }, [api]);
      ...
      <Container>
        <div className="flex flex-col items-center text-center mb-12">
          ...
          <p className="mt-6 text-lg text-muted-foreground max-w-[800px]">
            Example testimonials based on the value I aim to deliver to clients.
          </p>
          <p className="mt-2 text-sm text-muted-foreground max-w-[800px]">
            (These are demonstration testimonials to illustrate the types of results I work to achieve)
          </p>
        </div>
        ...
        <div className="mt-16 max-w-3xl mx-auto text-center">
          <div className="p-6 rounded-lg border border-border">
            ...
            <p className="text-muted-foreground mb-2">
              Let me help you streamline your operations with custom automation solutions.
            </p>
